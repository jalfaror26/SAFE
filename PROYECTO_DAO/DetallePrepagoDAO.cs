...
namespace PROYECTO_DAO
{
    public class DetallePrepagoDAO
    {
        ...
        {
            String sql = "SELECT distinct DETPRE_PREPAGO guia, PROV_NOMBRE proveedor, DETPRE_FACTURA factura , DETPRE_MONTO monto , DETPRE_SALDO saldo , DETPRE_ABONO abono , TO_CHAR(sysdate,'DD/MM/YYYY') fecha , PRE_MONEDA moneda , GAS_NOMBRE gasto FROM TBL_DETALLE_PREPAGO DPP , TBL_PREPAGOS_PROVEEDORES PPP , TBL_PROVEEDOR p , TBL_GASTOS g , TBL_FACTURAS_PENDIENTE_PAGO FPP WHERE dpp.no_cia = '" + pNo_cia + "' and dpp.no_cia = ppp.no_cia and dpp.no_cia = p.no_cia and dpp.no_cia = g.no_cia and dpp.no_cia = fpp.no_cia and DETPRE_FACTURA = FACPAG_NUM_FACTURA AND GAS_CODIGO = FACPAG_TIPO_GASTO AND prov_linea =PRE_PROVEEDOR AND pre_id = '" + prepago + "' AND detpre_prepago = pre_id AND DETPRE_ESTADO ='FP' AND pre_moneda ='" + moneda + "' AND upper(DETPRE_PROVEEDOR)=upper('" + proveedor + "') and facpag_proveedor= pre_proveedor and facpag_estado = 1";
            DataSet oDataSet = OracleDAO.getInstance().EjecutarSQLDataSet(sql);
            return oDataSet;
        }
