...
namespace PROYECTO_DAO
{
    public class RecordatorioDAO
    {
        public Boolean Guardar(Recordatorio oRecordatorio)
        {
            ...
            oCommand.Parameters[5].Value = oRecordatorio.Tipo;
            oCommand.Parameters.Add("persona", OracleType.Number);
            oCommand.Parameters[6].Value = oRecordatorio.Persona;
            oCommand.Parameters.Add("pNo_cia", OracleType.NVarChar);
            oCommand.Parameters[7].Value = oRecordatorio.No_cia;
        ...
        public Boolean Eliminar(Recordatorio oRecordatorio)
        {
            ...
            oCommand.Parameters[0].Value = oRecordatorio.Linea;
            oCommand.Parameters.Add("pNo_cia", OracleType.NVarChar);
            oCommand.Parameters[1].Value = oRecordatorio.No_cia;
        ...
        public DataSet Consultar(DateTime fecha, String pNo_cia)
        {
            String sql = "select rec_indice,rec_detalle, TO_CHAR(REC_FECHA,'HH:mi am') hora FROM TBL_RECORDATORIO r where r.no_cia = '" + pNo_cia + "' and  rec_usuario = user and rec_estadoregistro=1 and rec_estado='PEN' and trunc(rec_fecha)='" + fecha.ToShortDateString() + "' order by rec_fecha";
