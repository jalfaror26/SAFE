...
namespace PROYECTO_DAO
{
    public class CajaChicaDetalleDAO
    {
        public Boolean Insertar(CajaChicaDetalle oCajaChicaDetalle, String pNo_cia)
        {
            ...
            oCommand.Parameters[6].Value = oCajaChicaDetalle.Usuario;
            oCommand.Parameters.Add("tipomovimiento", OracleType.NVarChar);
            oCommand.Parameters[7].Value = oCajaChicaDetalle.TipoMovimiento;
            oCommand.Parameters.Add("justificacion", OracleType.NVarChar);
            oCommand.Parameters[8].Value = oCajaChicaDetalle.Justificacion;
            oCommand.Parameters.Add("pNo_cia", OracleType.NVarChar);
            oCommand.Parameters[9].Value = pNo_cia;
        ...
        {
            String sql = "SELECT  DETCAJ_FECHAMOVIMIENTO, DETCAJ_EMPLEADO,'CAJA CHICA' emp_nombre, DETCAJ_DOCUMENTO , DETCAJ_CREDITO, DETCAJ_DEBITO, DETCAJ_MOVIMIENTO, DETCAJ_JUSTIFICACION FROM TBL_CAJA_CHICA_DETALLE, tbl_caja_chica where DETCAJ_CAJA = CAJ_LINEA and CAJ_ESTADO =1 and caj_usuarioabre = '" + usuario + "'  and no_cia = '" + pNo_cia + "' order by DETCAJ_FECHAMOVIMIENTO, DETCAJ_MOVIMIENTO";
            DataSet oDataSet = OracleDAO.getInstance().EjecutarSQLDataSet(sql);
            return oDataSet;
        }
        ...
        {
            String sql = "SELECT CAJ_LINEA, CAJ_MONTO, CAJ_SALDO FROM tbl_caja_chica where CAJ_ESTADO =1 and caj_usuarioabre = '" + usuario + "' and no_cia = '" + pNo_cia + "'";
            DataSet oDataSet = OracleDAO.getInstance().EjecutarSQLDataSet(sql);
            return oDataSet;
        }
        ...
        public string Moneda(String usuario, String pNo_cia)
        {
            String sql = "SELECT CAJ_Moneda FROM TBL_CAJA_CHICA where CAJ_ESTADO=1 and caj_usuarioabre = '" + usuario + "' and no_cia = '" + pNo_cia + "'";
