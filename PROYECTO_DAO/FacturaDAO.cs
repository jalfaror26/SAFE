...
namespace PROYECTO_DAO
{
    public class FacturaDAO
    {
        ...
        {
            String sql = "select fac_numero||'  -  ' ||fac_estado as cod,fac_fecha || '  -  ' || fac_nombre as descripcion from TBL_FACTURA F where f.no_cia = '" + pNo_cia + "' order by to_number(fac_numero) desc";
            DataTable oDataTable = OracleDAO.getInstance().EjecutarSQLDataTable(sql);
            return oDataTable;
        }
        ...
        {
            String sql = "select fac_numero ||'  -  ' ||fac_estado as cod,FAC_FECHA || '  -  ' || fac_nombre as descripcion from TBL_FACTURA F where f.no_cia = '" + pNo_cia + "' and ";
            if (tipo == 1)
